...
import ChartComponent from "@/components/nft/ChartComponentAlt";
...
import { getNameFromRri, getPrice, getPriceText } from "@/lib/utils";
...
export default function Nft({ nft, collection, similarNfts, owner }) {
  ...
  const handleCreateAuction = async () => {
    ...
    // message.warning(
    //   `start auction for nft #${nft.referenceId} for ${
    //     // days difference
    //     Math.floor(
    //       (new Date(auctionEndDate).getTime() - new Date().getTime()) /
    //         (1000 * 3600 * 24)
          ...
          {nft.attributes?.length > 0 ? (
            <div className="grid grid-cols-2 gap-2 xl:grid-cols-3">
              {nft.attributes?.map((attribute) => {
                return (
                  <div className="border border-solid border-[#CFDBD599] rounded-[12px] text-center py-2 px-3">
            ...
            <div className="w-full md:w-1/2 lg:pr-[48px] md:pr-[24px] sm:pr-[24px] pr-[5px]">
              <div className="flex items-center justify-between w-full">
                <div
                  ...
                  onClick={() => {
                    if (!root.user || !session) {
                      message.error("Please login to favorite NFT");
                      router.push("/auth/login");
                    }
                    ...
                    toggleLike({
                      ...
                      if (res.data?.hasLiked) {
                        root.socket.emit("sendNotification", {
                          ...
                          message: {
                            type: "favourite",
                            text: `${root.user?.username} favourited your NFT`,
                            link: `/nft/${nft._id}`,
                          },
                        });
                      }
                  ...
                  <div className=" w-[140px] h-[43px] bg-white dark:bg-secondaryBlack text-secondaryBlack rounded-[12px] left-4 top-4 px-3 flex items-center justify-between gap-x-2 !text-xs font-bold border border-solid border-[#CFDBD599]">
                    ...
                    {nft?.endDate && (
                      <div className="w-[100px]">
                        ...
                        <Statistic.Countdown
                          value={
                            // new Date(nft?.endDate) ||
                            // 2 days from now
                            // Date.now() + 1000 * 60 * 60 * 24 * 2
                            // date the nft endDate with the same format above
                            new Date(nft?.endDate).getTime()
                          }
                          format="DD:HH:mm:ss"
                          onFinish={() => {
              ...
              <div className="mt-6">
                ...
                {/* <div className=" my-2 font-bold text-sm text-secondaryDarkGray bg-secondary w-fit px-2 py-1">
                  Top:{" "}
                  <span className="text-secondaryBlack dark:text-white">
                    {
                      // total rarity based on every trait percantage in attributes array
                      nft.attributes?.reduce((acc, curr) => {
                        return acc + curr.percentage;
                      }, 0) / nft.attributes?.length
                    }
                    %
                  </span>
                </div> */}
                ...
                {nft.buyType === "random" &&
                  ...
                  nft.ownerWallet === "Locked" && (
                    <p className="my-2 text-sm font-semibold">
                      Please Note: This collection has a random minting and you
                      will not be guaranteed the NFT you see.
                    </p>
                  )}
        ...
        <div>
          <div className="w-full mt-4">
            ...
            <Input
              ...
              // suffix={<MdMailOutline className="text-secondaryGray w-5 h-5" />}
              style={{
                borderRadius: "24px",
              }}
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        <div className="flex items-center gap-x-2">
                          <Image
                            src={
                              bid?.bidder?.profilePicture || profilePlaceholder
                            }
                            width={40}
                            height={40}
                            className={`rounded-full ${
            ...
            <Button
              ...
              onClick={async function () {
                ...
                if (initalBidAmount < 2) {
                  return message.error("Bid amount must be more than 1 XRD");
                }
          ...
          <div className="flex justify-center gap-x-5">
            ...
            <Button
              ...
              onClick={async function () {
                ...
                // find the bidder using wallet in bids
                // const bidder = nft.bids.find(
                //   (bid) => bid.wallet === root.user?.wallet
                // );

                // if (bidder && higherBidAmount <= bidder.bid) {
                //   return message.error(
                //     `Bid amount must be more than your previous bid of ${bidder.bid}`
                //   );
                // }
